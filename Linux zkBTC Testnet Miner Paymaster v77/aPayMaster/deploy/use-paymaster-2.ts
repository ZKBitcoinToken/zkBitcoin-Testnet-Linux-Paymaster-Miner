	...
	try{
			...
			//	data22Nonce=  data22Nonce.slice(0, 3050); // Slice the array to exclude the last 'x' elements
				...
				PAYMASTER_ADDRESS = config2.contractAddressPayMaster;
				  ...
				  var doEz = false;
				  if(count >= 20 && gasLimitBump == 89){gasLimitBump = 85;}
  ...
  try {
    ...
    // Checking if the error message contains the text 'minAmt'
	  ...
	  console.log("Error contains 'Not enough gas for transaction', Means we need to UP the Gas Price, doing that now and resending");
				 /*gasLimitBump = gasLimitBump + Math.floor(gasLimitBump*3/100);
				 if(gasLimitBump < 90){
				  gasLimitBump = gasLimitBump + Math.floor(gasLimitBump*3/100);
		...
		console.log("gas Limit bumped to: "+gasLimitBump.toString());
		*/
					await sleep(100); // Sleep for 2 seconds (2000 milliseconds)
					var transaction;
					
					var gasLimitUpdate = Math.floor(gasLimit*1015/1000);
					if(gasLimitUpdate > gasLimit){
						gasLimit = gasLimitUpdate;
					}
					gasLimitBump = Math.floor(gasLimitBump*1015/1000);
					var xxxxxx=0;
					while(xxxxxx<10){
							try{
								const paymasterParams = utils.getPaymasterParams(PAYMASTER_ADDRESS, {
							  ...
							  transaction = await erc20.multiMint_PayMaster_EZ(config2.minerAddress, data22Nonce, {
							  // paymaster info
							  customData: {
								gasPerPubdata: utils.DEFAULT_GAS_PER_PUBDATA_LIMIT,
								paymasterParams: paymasterParams,
								  ...
								  gasPrice: gasPrice, // Set the custom gas price
									...
									//console.error(err);
									console.log("Too little gas Probably upping it more, current GasLimit: "+gasLimit);
									//console.error(err);
									var gasLimitUpdate = Math.floor(gasLimit*1015/1000);
									gasLimitBump = Math.floor(gasLimitBump*1015/1000);
									if(gasLimitUpdate > gasLimit){
	...
	*/
				...
				data22Nonce= null;
				 ...
				 await sleep(400); // Sleep for 2 seconds (2000 milliseconds)
